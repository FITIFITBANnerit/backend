...
public class ReportController {
    ...
    public ResponseEntity<SaveReportResponse> createReport(@RequestHeader("Authorization") String authorizationHeader,
        ...
        try {
            ...
            SaveReportResponse errorResponse = new SaveReportResponse(new ErrorDetails(400, "Bad Request: Invalid or missing token"), null);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
        } catch (Exception e) {
            // 그 외 예외 처리
            SaveReportResponse errorResponse = new SaveReportResponse(new ErrorDetails(500, "Internal Server Error"), null);
    ...
    public ResponseEntity<ReportLogsResponse> findReportLogsByUser(@RequestHeader("Authorization") String authorizationHeader){
        try {
            ...
            // 잘못된 Authorization 헤더 또는 토큰 오류 처리
            ReportLogsResponse errorResponse = new ReportLogsResponse(new ErrorDetails(400, "Bad Request: Invalid or missing token"), null);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
        } catch (Exception e) {
            // 그 외 예외 처리
            ReportLogsResponse errorResponse = new ReportLogsResponse(new ErrorDetails(500, "Internal Server Error"), null);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(errorResponse);
        }
    }
